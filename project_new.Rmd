...
-For our data we found a data set on Kaggle.com. The data set is the box score for all NBA games since 2014. The data set has 51 variables and 155,489. 
...
-This new data set contains the box score for all players on the Suns when they have played the Bucks since 2014.

-We looked at another box plot of points per minute vs positions for both home and away games. The graph looks similar to the points per minute by position for all games since 2014.
...
-For the number of simulations requested, and for an individual player, the function randomly samples from the distributions we have built for them. It takes one sample of how long they played, multiplied by one sample of how many points they score per minute. This whole process is one game’s simulation
...
```{r}
gsw_cle_games <- c(204, 245, 231, 253, 249, 238, 225, 212, 193)
mean(gsw_cle_games)
sd(gsw_cle_games)
```
